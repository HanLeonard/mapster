...
public unsafe class DataFile : IDisposable {
    ...
    // OWN-CODE
    public string[] CustomPropertyEnumString = new[] {
            ...
            for (var j = 0; j < header.Tile.Value.FeaturesCount; ++j) {
                ...
                if (isFeatureInBBox)
                {
                    ...
                    for (var p = 0; p < feature->PropertyCount; ++p) {
                        ...
                        if (CustomPropertyEnumString.Contains(key.ToString())) {
                            try {
                                // Convert the string obtained from the file to an equivalent enum type for easier computation
                                MapFeatureData.CustomPropertyEnum keyEnum =
                                    (MapFeatureData.CustomPropertyEnum)Enum.Parse(
                                        typeof(MapFeatureData.CustomPropertyEnum),
                                        FirstCharToUpper(key.ToString()));
